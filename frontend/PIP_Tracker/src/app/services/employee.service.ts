...
export class EmployeeService {
  ...
  verifyOtpAndReset(email: string, otp: string, newPassword: string): Observable<any> {
    ...
    return this.http.put(`${this.apiUrl}/employees/reset-password/verify-otp-reset`, payload, {
      ...
      responseType: 'text' as 'json', // 👈 This line is critical
  ...
  addSkillGap(skillGapData: any, token: string) {
    ...
    return this.http.post(`${this.apiUrl}/skill-gap`, skillGapData, {
      headers,
      responseType: 'text' as 'json'   // 👈 plain text accept karega
    });
  }
  ...
  // getFeedbacksReceived(userId: string, token: string): Observable<any[]> {
  //   const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
  //   const url = `${this.apiUrl}/${userId}/feedbacks`;
  ...
  // addToPip(id: string, token: string): Observable<any> {

  //   const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);

  //   return this.http.put(`${this.apiUrl}/employees/add-to-pip/${id}`, null, { headers });
  ...
  // EmployeeService ke andar ka method
  // getFeedbacksReceived(userId: string, token: string): Observable<any[]> {
  //   const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
  //   // ✅ Yahan userId variable ka istemaal kiya hai, jaisa ki hona chahiye.
  //   const url = `${this.apiUrl}/feedbacks/toUser/${userId}`; 
